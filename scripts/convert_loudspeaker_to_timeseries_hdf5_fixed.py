...
def _write_partitioned_asdf_h5(
    ...
    *,
    ...
    with h5py.File(output_path, "w") as f:
        ...
        # Create variance group and store variance data as datasets
        variance_group = group.create_group("variance")
        variance_group.attrs["description"] = _as_h5_str(
            "Variance metadata from repeated measurements. "
            "mean_variance: average variance per channel across time. "
            "pre_variance: noise floor variance before excitation. "
            "post_variance: noise floor variance after excitation."
        )
        variance_group.attrs["channel_names_json"] = _as_h5_str(
            json.dumps(["voltage", "current", "displacement", "velocity"])
        )
        variance_group.attrs["channel_units_json"] = _as_h5_str(
            json.dumps(["V^2", "A^2", "m^2", "(m/s)^2"])
        )

        # Collect variance data from all measurements
        mean_variances = []
        pre_variances = []
        post_variances = []
        for m in measurements:
            if "mean_var" in m:
                mean_variances.append(np.asarray(m["mean_var"], dtype=np.float64))
            if "pre_var" in m:
                pre_variances.append(np.asarray(m["pre_var"], dtype=np.float64))
            if "post_var" in m:
                post_variances.append(np.asarray(m["post_var"], dtype=np.float64))

        if mean_variances:
            variance_group.create_dataset(
                "mean_variance",
                data=np.stack(mean_variances, axis=0),
            )
        if pre_variances:
            variance_group.create_dataset(
                "pre_variance",
                data=np.stack(pre_variances, axis=0),
            )
        if post_variances:
            variance_group.create_dataset(
                "post_variance",
                data=np.stack(post_variances, axis=0),
            )
        ...
        for (signal, duration_s, samplerate), items in sorted(variants.items(), key=lambda k: str(k[0])):
            ...
            for m_idx, m in enumerate(items):
                ...
                for k, v in m.items():
                    if k in {"input_data", "input_data_var", "output_data", "mean_var", "pre_var", "post_var"}:
                        continue
                    attrs[k] = v
